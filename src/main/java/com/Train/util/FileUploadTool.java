...
public class FileUploadTool {
	...
	private static String[] allowFiles = { ".rar", ".doc", ".docx", ".zip", ".pdf", ".txt", ".swf", ".xlsx", ".gif",
			".png", ".jpg", ".jpeg", ".bmp", ".xls", ".flv",".mp4",".ppt", ".avi", ".mpg", ".wmv", ".3gp", ".mov",
	...
	public FileEntity createFile(MultipartFile multipartFile, HttpServletRequest request) {
		...
		if (bflag) {
			...
			String finalFileDir = builder.toString();
			// size存储为String
	...
	public Course createImg(MultipartFile multipartFile, HttpServletRequest request) {
		...
		if (bflag) {
			String logoPathDir = "/upload/image/";
			//String logoRealPathDir = request.getSession().getServletContext().getRealPath(logoPathDir);
			// 上传到本地磁盘
			String logoRealPathDir = "E:/upload/image";
			File logoSaveFile = new File(logoRealPathDir);
			if (!logoSaveFile.exists()) {
				logoSaveFile.mkdirs();
			}
