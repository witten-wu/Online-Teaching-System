...
<div class="container">
	...
	<div class="row clearfix" id="List">
		<div class="col-md-12" >
		<input type="text" style="margin-right:500px;font-size:16px;background:none; border:none;" value="课程章节" disabled/>
	...
	<div class="col-md-12" id="Exam" style="margin: 20px 0 0 0 " hidden>
				...
				<button class="btn btn-warning btn-lg btn-block" style="width:300px;height:45px;margin:50px 0 15px 0;margin-left:auto;margin-right:auto;" id="sureAlter">答题结束</button>
         	</div>
...
function takeexam(obj){ 
	...
	if(Score==100){
		...
		$("div#info").show();
		$("#result").val("共答对"+Score/20+"题，答错题目如下：")
	}
	$("div#button").show();
	});
};
...
function openmovie(obj){ 
	var fileId=obj.id;
	window.open('myCourses3.jsp?'+fileId)
};
	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0;i<dataList.length;i++){
				...
				button2.className="btn btn-success btn-xs";
				
				button1.append(str1);
				button2.append(str2);
				
				button1.id=dataList[i].fileId;
				button2.id=dataList[i].fileId;
